...
import { snapshot, stripTrailingSlash } from './helpers';
...
var SpecManager = (function () {
    ...
    SpecManager.prototype.preprocess = function () {
        ...
        if (this._schema.securityDefinitions && !this.options.noAutoAuth) {
            var SecurityDefinitions = require('../components/').SecurityDefinitions;
    ...
    SpecManager.prototype.getAuthHeader = function (operationPointer, authorized) {
        // TODO: only oAuth2 covered now
        var operation = this.byPointer(operationPointer);
        var securities = operation.security || this.schema.security;
        var defs = this.schema.securityDefinitions || {};
        if (!securities)
            return;
        var result = '';
        securities.forEach(function (securityObj) {
            for (var key in securityObj) {
                ...
                if (type === 'oauth2') {
                    result = (tokenType || 'Bearer') + " " + accessToken;
                    return;
                }
            }
        });
    ...
    SpecManager.prototype.getBaseUrl = function (hideHostname) {
        if (hideHostname) {
            return '';
        }
        else {
    ...
    SpecManager.prototype.getServers = function (hideHostname) {
        var _this = this;
        var xServers = this._schema['x-servers'];
        var res;
        if (xServers) {
            res = xServers.map(function (_a) {
                var url = _a.url, description = _a.description;
                return ({
                    description: description,
                    url: stripTrailingSlash(url.startsWith('//') ? _this.apiProtocol + ":" + url : url)
                });
            });
        }
        else {
    ...
    SpecManager.prototype.specWithServer = function (serverUrl) {
        var _a = urlParse(serverUrl), host = _a.host, basePath = _a.path, protocol = _a.protocol;
        return __assign({}, this._schema, { host: host,
            basePath: basePath, schemes: [protocol.replace(':', '')] });
    };
    return SpecManager;
}());
export { SpecManager };
